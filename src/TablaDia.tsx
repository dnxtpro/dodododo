...
const TablaDia = () => {
  ...
  const handleCreateEvent = (e: React.FormEvent) => {
    ...
    if (newTask.Titulo && newTask.Fecha_Inicio && newTask.Fecha_Fin) {
      const createdEvent: Tarea = {
        ...
        Categoria: {
          id: newTask.Categoria.id,
          name: newTask.Categoria.name,
          color: newTask.Categoria.color,
        },
    ...
    const scrollToCurrentTime = () => {
      if (timeContainerRef.current) {
        ...
        const scrollPosition =
          (totalMinutes / 1440) * timeContainerRef.current.scrollHeight;
        timeContainerRef.current.scrollTop =
          scrollPosition - timeContainerRef.current.clientHeight / 2;
      }
    };
  ...
  const renderPlaceholderEvent = () => {
    ...
    const startOffsetInMinutes = differenceInMinutes(
      startDate,
  ...
  useEffect(() => {
    ...
    // Añadir evento al montar el componente
  ...
  return (
    <div
      className="  border-b-4 max-h-screen overflow-auto h-[100vh]"
      ref={timeContainerRef}
    >
      <div className="sticky top-0 z-10">
        <div className="flex justify-around items-center p-4 bg-primary text-primary-foreground">
          <Button onClick={handlePrevWeek} variant="ghost" size="icon">
            <ChevronLeft className="h-4 w-4" />
          </Button>
          <Sheet>
            <SheetTrigger asChild>
              <Button onClick={handleHorarioForm}>
                <Plus />
            ...
            <SheetContent className="bg-secondary">
              <SheetHeader>
                ...
                <SheetDescription>Añade tus horarios fijos</SheetDescription>
              </SheetHeader>
              <form onSubmit={handleCreateEvent} className="space-y-4">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      ...
                      if (selectedCategory) {
                        ...
                        }); // Establece la categoría completa
                      }
                    }}
                  >
                ...
                >
                  <div className="mx-auto">
                    ...
                    <div className="relative ">
                      <DatePicker
                        selected={
                          newTask.Fecha_Inicio
                            ? new Date(newTask.Fecha_Inicio)
                            : null
                        } // Muestra la Fecha_Inicio
                        onChange={(time: Date | null) => {
                          if (time && newTask.Fecha_Inicio) {
                            const newFechaInicio = new Date(
                              newTask.Fecha_Inicio
                            ); // Crea una nueva fecha a partir de la Fecha_Inicio
                            newFechaInicio.setHours(
                              time.getHours(),
                              time.getMinutes()
                  ...
                  <div className="mx-auto">
                    ...
                    <div className="relative ">
                      <DatePicker
                        selected={
                          newTask.Fecha_Fin ? new Date(newTask.Fecha_Fin) : null
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    value={
                      newTask.Fecha_Fin
                        ? format(newTask.Fecha_Fin, "dd/MM/yyyy HH:mm")
                        : ""
                    }
                    disabled
                  />
        ...
        <div className="flex flex-row  bg-white z-10">
          ...
          {weekDays.map((day, index) => (
            <div
              className={`flex-1  min-w-[150px] flex items-center justify-center font-semibold ${
                isToday(day)
        ...
        {weekDays.map((day, idx) => (
          <div className="flex-1 flex flex-col relative" key={idx}>
            {timeSlots.map((slot, index) => {
              ...
              return (
                <div
                  className="flex-1  border-l border-b border-gray-300 relative hover:bg-gray-400/20"
                  key={index}
                ...
                >
                  ...
                  {events.map((event, eventIndex) => {
                    ...
                    const isEventInSlot = isWithinInterval(slotStart, {
                      start: eventStart,
                      end: eventEnd,
                    });
                    ...
                    if (isEventInSlot && isFirstSlot) {
                      console.log(
                        `Rendering event ${eventIndex} on day ${format(
                          day,
                          "yyyy-MM-dd"
                        )} in slot ${index}`
                      );
                      ...
                      const overlappingEvents = events.filter(
                        (e) =>
                          ...
                          isWithinInterval(e.Fecha_Inicio, {
                            start: eventStart,
                            end: eventEnd,
                          }) ||
                          isWithinInterval(e.Fecha_Fin, {
                            start: eventStart,
                            end: eventEnd,
                          })
                      );
                ...
                <form onSubmit={handleCreateEvent} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      value={
                        newTask.Fecha_Inicio
                          ? format(newTask.Fecha_Inicio, "dd/MM/yyyy HH:mm")
                          : ""
                      }
                      disabled
                    />
